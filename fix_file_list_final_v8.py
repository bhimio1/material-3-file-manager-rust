...
fn render_file_item<C: VisualContext>(
    item: &FileEntry,
    index: usize,
    is_grid: bool,
    ...
    if is_image && thumbnail_path.is_none() {
        // Warning: using cx inside async block might need spawn on context?
        // But FileListEntity pending_thumbnails is Model/View state.
        // We can't access `file_list_entity.read(cx)` inside `render_file_item` generic C?
        // VisualContext has `read_model`?
        // `file_list_entity` is `Entity<FileList>`.
        // `Entity` methods need `&AppContext` (which VisualContext derefs to).

        // However, `read` returns Ref, which is bound to lifetime of borrow.
        // Let's assume Entity::read works on VisualContext.
        ...
        if !is_pending {
            ...
            // cx.background_executor() is available on AppContext
